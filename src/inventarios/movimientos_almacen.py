    ...
    if 'operation_x' in request.POST.keys():
        ...
        if operation == 'stock_producto':
            ...
            if settings.PRODUCTOS_USAR_FECHAS and settings.PRODUCTOS_USAR_LOTE:
                return render(request, 'inventarios/stock_producto.html', context_stock)
            else:
                if not settings.PRODUCTOS_USAR_FECHAS and not settings.PRODUCTOS_USAR_LOTE:
                    ...
                    if settings.PRODUCTOS_USAR_FECHAS:
                        return render(request, 'inventarios/stock_producto_solo_fecha.html', context_stock)
                    if settings.PRODUCTOS_USAR_LOTE:
                        return render(request, 'inventarios/stock_producto_solo_lote.html', context_stock)
...
def movimientos_almacen_add(request):
    ...
    lista_almacenes_destino = lista_controller.get_lista_almacenes(user=request.user, module=settings.MOD_MOVIMIENTOS_ALMACEN)
    ...
    if settings.PRODUCTOS_USAR_FECHAS and settings.PRODUCTOS_USAR_LOTE:
        return render(request, 'inventarios/movimientos_almacen_form.html', context)
    else:
        if not settings.PRODUCTOS_USAR_FECHAS and not settings.PRODUCTOS_USAR_LOTE:
            return render(request, 'inventarios/movimientos_almacen_form_sin_fechas_lote.html', context)
        else:
            if settings.PRODUCTOS_USAR_FECHAS:
                return render(request, 'inventarios/movimientos_almacen_form_solo_fecha.html', context)
            if settings.PRODUCTOS_USAR_LOTE:
                return render(request, 'inventarios/movimientos_almacen_form_solo_lote.html', context)
