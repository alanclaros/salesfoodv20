...
# cajas entregar recibir index
@user_passes_test(lambda user: get_user_permission_operation(user, settings.MOD_ENTREGAR_CAJA_RECIBIR, 'lista'), 'without_permission')
def cajas_entregar_recibir_index(request):
    # permisos
    permisos = get_permissions_user(request.user, settings.MOD_ENTREGAR_CAJA_RECIBIR)
    ...
    # entregar recibir caja
    if 'operation_x' in request.POST.keys() and request.POST['operation_x'] == 'entregar_recibir_x':
        if permisos.adicionar:
            try:
                caja = Cajas.objects.select_related('punto_id').get(pk=int(request.POST['id']))

                if caja_controller.add_operation('entregar_recibir', current_date(), caja, formato_ori='yyyy-mm-dd'):
                    ...
                    context = {
                        ...
                        'module_x': settings.MOD_ENTREGAR_CAJA_RECIBIR,
                        'module_x2': '',
                        'module_x3': '',

                        'operation_x': 'entregar_recibir',
                        'operation_x2': '',
                        'operation_x3': '',
                    ...
                    return render(request, 'cajas/cajas_entregar_recibir_form.html', context)
                else:
                    messages.add_message(request, messages.SUCCESS, {'type': 'warning', 'title': 'Cajas!', 'description': 'la caja debe estar en estado cierre'})

            except Exception as ex:
                messages.add_message(request, messages.SUCCESS, {'type': 'warning', 'title': 'Cajas!', 'description': 'error al aceptar el cierre de caja, ' + str(ex)})

        else:
            return render(request, 'pages/without_permission.html', {})

    # entregar recibir registrar
    if 'operation_x' in request.POST.keys() and request.POST['operation_x'] == 'entregar_recibir_guardar_x':
        if permisos.adicionar:
            try:
                caja = Cajas.objects.get(pk=request.POST['id'])

                if caja_controller.save_data(current_date(), caja, 'entregar_recibir_guardar', request, formato_ori='yyyy-mm-dd'):
                    # guardado
                    messages.add_message(request, messages.SUCCESS, {'type': 'success', 'title': 'Cajas!', 'description': 'Se cerro la Caja: ' + caja.caja})
                else:
                    messages.add_message(request, messages.SUCCESS, {'type': 'warning', 'title': 'Cajas!', 'description': 'la caja debe estar en estado cierre'})

            except Exception as ex:
                messages.add_message(request, messages.SUCCESS, {'type': 'warning', 'title': 'Cajas!', 'description': 'error al registrar la recepcion de caja, ' + str(ex)})

        else:
            return render(request, 'pages/without_permission.html', {})

    # entregar recibir caja, cancelar
    if 'operation_x' in request.POST.keys() and request.POST['operation_x'] == 'cancelar_entregar_recibir_x':
        if permisos.eliminar:
            try:
                caja = Cajas.objects.select_related('punto_id').get(pk=int(request.POST['id']))

                if caja_controller.add_operation('entregar_recibir_cancelar', current_date(), caja, formato_ori='yyyy-mm-dd'):
                    """si la caja esta en el estado correcto"""
                    monedas = caja_controller.get_coins(current_date(), caja, formato_ori='yyyy-mm-dd')
                    # detalles
                    operacion = CajasOperaciones.objects.filter(caja_id=caja, fecha=current_date())[:1].get()
                    ...
                    context = {
                        ...
                        'operation_x': 'entregar_recibir_cancelar',
                        'operation_x2': '',
                        'operation_x3': '',
                    ...
                    messages.add_message(request, messages.SUCCESS, {'type': 'warning', 'title': 'Cajas!', 'description': 'la caja debe estar en estado cierre-recibe'})
    ...
    if 'operation_x' in request.POST.keys() and request.POST['operation_x'] == 'cancelar_entregar_recibir_guardar_x':
        if permisos.eliminar:
            try:
                caja = Cajas.objects.get(pk=request.POST['id'])

                if caja_controller.save_data(current_date(), caja, 'entregar_recibir_cancelar', request, formato_ori='yyyy-mm-dd'):
                    # guardado
                    messages.add_message(request, messages.SUCCESS, {'type': 'success', 'title': 'Cajas!', 'description': 'Se cancelo el cierre de la Caja: ' + caja.caja})
                else:
                    messages.add_message(request, messages.SUCCESS, {'type': 'warning', 'title': 'Cajas!', 'description': 'la caja debe estar en estado entrega-recibe para cancelarlo'})

            except Exception as ex:
                messages.add_message(request, messages.SUCCESS, {'type': 'warning', 'title': 'Cajas!', 'description': 'error al registrar la cancelacion de recepcion de cierre de caja, ' + str(ex)})
    ...
    cajas = caja_controller.index_operations(user_perfil, request.user, 'entregar_recibir')
    # print(cajas)
    ...
    return render(request, 'cajas/cajas_entregar_recibir.html', context)
