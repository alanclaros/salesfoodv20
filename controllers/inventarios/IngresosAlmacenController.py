...
class IngresosAlmacenController(DefaultValues):
    def __init__(self):
        DefaultValues.__init__(self)
        self.modelo_name = 'Registros'
        self.modelo_id = 'registro_id'
        self.modelo_app = 'inventarios'
        self.modulo_id = settings.MOD_INGRESOS_ALMACEN

        # variables de session
        self.modulo_session = "ingresos_almacen"
    ...
    def index(self, request):
        ...
        # tipo
        self.filtros_modulo['tipo_movimiento'] = 'INGRESO'
        ...
        #print('ingresos almacen: ', self.filtros_modulo)
    ...
    def save(self, request, type='add'):
        ...
        try:
            ...
            for i in range(1, 51):
                ...
                aux_2 = request.POST['fecha_elaboracion_' + str(i)].strip()
                aux_3 = request.POST['fecha_vencimiento_' + str(i)].strip()
                aux_4 = request.POST['lote_' + str(i)].strip().upper()
                ...
                if aux != '0' and aux_5 != '' and aux_6 != '':
                    if aux_2 == '':
                        ...
                        fecha_elaboracion = get_date_to_db(formato='yyyy-mm-dd HH:ii:ss', formato_ori='dd/MM/yyyy', fecha=aux_2)

                    if aux_3 == '':
                        fecha_vencimiento = None
                    else:
                        fecha_vencimiento = get_date_to_db(formato='yyyy-mm-dd HH:ii:ss', formato_ori='dd/MM/yyyy', fecha=aux_3)
                    ...
                    dato_detalle['lote'] = aux_4
                    dato_detalle['cantidad'] = Decimal(aux_5)
        ...
        try:
            ...
            with transaction.atomic():
                ...
                for detalle in datos['detalles']:
                    ...
                    stock_up = stock_controller.update_stock(user_perfil=datos['user_perfil_id'], almacen=datos['almacen_id'], producto=detalle['producto_id'], cantidad=detalle['cantidad'],
    ...
    def can_anular(self, id, usuario_perfil):
        ...
        try:
            ...
            registro = apps.get_model('inventarios', 'Registros').objects.get(pk=id)
            permisos = get_permissions_user(usuario_perfil.user_id, settings.MOD_INGRESOS_ALMACEN)
            ...
            if self.can_anular(id, datos['user_perfil_id']):
                ...
                with transaction.atomic():
                    ...
                    for registro_detalle in registros_detalles:
                        stock_up = stock_controller.update_stock(user_perfil=datos['user_perfil_id'], almacen=registro.almacen_id, producto=registro_detalle.insumo_id, cantidad=(
                            0-registro_detalle.cantidad), fecha_elaboracion=registro_detalle.fecha_elaboracion, fecha_vencimiento=registro_detalle.fecha_vencimiento, lote=registro_detalle.lote)
