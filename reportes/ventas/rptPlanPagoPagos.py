...
def rptPlanPagoPagos(buffer_pdf, usuario, plan_pago_id, crear_pdf='si'):
    ...
    #plan_pago_detalles = PlanPagosDetalles.objects.filter(plan_pago_id=plan_pago).order_by('numero_cuota')
    status_cuota = Status.objects.get(pk=settings.STATUS_CUOTA_PAGADA)
    plan_pago_detalles = PlanPagosPagos.objects.filter(plan_pago_id=plan_pago, status_id=status_cuota).order_by('fecha')

    # verificamos si esta anulado
    dato_anulado = ''
    if plan_pago.status_id.status_id == settings.STATUS_ANULADO:
        usuario_perfil = UsersPerfiles.objects.get(pk=plan_pago.user_perfil_id_anula)
        motivo_anula = plan_pago.motivo_anula
    ...
    for detalle in plan_pago_detalles:
        cuota = str(detalle.numero_cuota)
        fecha_plan = get_date_show(fecha=detalle.fecha, formato='dd-MMM-yyyy')
        monto = str(round(detalle.monto, 2))
        saldo = str(round(detalle.saldo, 2))
        observacion = Paragraph(detalle.persona_paga, style_tabla_datos)
